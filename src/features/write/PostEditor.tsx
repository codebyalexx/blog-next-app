...
import { createPost, editPost } from "@/src/action/post.action";
import { TElement } from "@udecode/plate-common";
import { PenIcon, SendIcon, Trash2Icon } from "lucide-react";
...
export const PostEditor = ({
  ...
  const [tags, setTags] = useState(
    (data ? data.tags.split(";") : []) as string[]
  );
    ...
    <>
      ...
      <div className="flex items-center justify-end">
        ...
        >
          {isLoading ? (
            <>
              {data ? "Uploading changes" : "Creating post"} <Loader />
            </>
          ) : (
            <>
              {data ? "Save modifications" : "Publish"}{" "}
              {data ? (
